...
SHELL_EXEMPT_COMMAND_NAMES = {
    ...
    "assert_matches", "assert_not_matches", "assert_null", "assert_not_null",
    "fail"
}
SHELL_EXEMPT_COMMAND_PREFIXES = ("stdlib.", "_stdlib", "_testing.", "docs.",
                                 "__", "_STDLIB_BINARY_", "$", "-")
...
class ProjectContext:
            ...
            for filename in files:
                if filename.endswith((".sh", ".snippet")):
                    with open(os.path.join(root, filename),
                              ...
                              encoding="utf-8") as f:
                        content = f.read()
                        matches = re.findall(REGEX_FUNCTION_DEFINITION,
    ...
    def _get_defined_binaries(self) -> Set[str]:
        ...
        if os.path.exists(PATH_BINARY_DEFINITIONS_FILE):
            with open(PATH_BINARY_DEFINITIONS_FILE, "r",
                      encoding="utf-8") as f:
                content = f.read()
...
class FileAuditor:
    ...
    def _is_exempt(self, cmd: str) -> bool:
        exemption_sources = [
            SHELL_KEYWORDS, self.context.builtins, self.local_scope,
            self.context.functions, SHELL_EXEMPT_COMMAND_NAMES
        ]
