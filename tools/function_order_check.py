...
class BashFile:
    ...
    def _parse(self):
        """
        Parses the file and extracts function definitions.
        """
        try:
    ...
    def get_sorting_errors(self) -> List[str]:
        """
        Checks if functions are in the correct custom alphabetical order.
        ...
        if func_names != sorted_names:
            return [
                f"Functions are not in alphabetical order (refining: Letters < _ < a-z < . < __).",
...
def main():
    ...
    if not files_to_check:
        print(
            "Error: No files provided for auditing. Please provide at least one file path.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if all_discrepancies:
        ...
        sorted_output = {
            k: all_discrepancies[k] for k in sorted(all_discrepancies.keys())
        }
