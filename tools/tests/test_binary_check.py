...
from binary_check import FileAuditor, LineAuditor, ProjectContext


class TestProjectContext(unittest.TestCase):
    @patch("subprocess.run")
    def test_get_bash_builtins__returns_set_of_builtins(
        self,
        mock_run,
    ):
    ...
    @patch("os.walk")
    def test_get_project_functions__returns_set_of_function_names(
        self,
        mock_walk,
    ):
        ...
        mock_run_patch = patch(
            "subprocess.run", return_value=MagicMock(stdout="")
        )
        mock_exists_patch = patch("os.path.exists", return_value=False)
    ...
    def test_get_defined_binaries__returns_set_of_binary_names(
        self,
        mock_exists,
    ):
        ...
        mock_walk_patch = patch("os.walk", return_value=[])
        mock_open_patch = patch(
            "builtins.open", mock_open(read_data=binary_content)
        )
...
class TestLineAuditor(unittest.TestCase):
    def test_sanitize__redirection_syntax__returns_removed_redirections(self):
    ...
    def test_sanitize__arithmetic_expansion__returns_sanitized_expansion(self):
    ...
    def test_split_segments__multiple_delimiters__returns_segments(self):
    ...
    def test_split_segments__nested_variable_expansion__returns_segment(self):
    ...
    def test_clean__shell_special_characters__returns_clean_commands(self):
        auditor = LineAuditor("")
    ...
    def test_find_commands__control_flow_keywords__returns_expected_list(self):
    ...
    def test_find_commands__negation_operator__returns_expected_list(self):
    ...
    def test_find_commands__subshell_assignment__returns_expected_list(self):
...
class TestFileAuditor(unittest.TestCase):
    def setUp(self):
        ...
        self.assets_dir = os.path.join(
            os.path.dirname(__file__), "assets", "binary_check"
        )

    def test_update_scope__local_declaration__adds_var_to_proper_scope(self):
    ...
    def test_update_scope__var_assignment__adds_var_to_proper_scope(self):
        auditor = FileAuditor("mock.sh", self.context)
    ...
    def test_check__unauthorized_absolute_path__adds_violation(self):
        ...
        self.assertIn(
            "Line 10: Direct call to absolute path '/usr/bin/ls'.",
            auditor.violations,
        )
    ...
    def test_check__defined_binary__adds_violation(self):
        ...
        self.assertIn(
            "Line 12: Direct call to defined binary 'cat'. "
            "Use _STDLIB_BINARY_CAT instead.",
            auditor.violations,
        )
    ...
    def test_check__unknown_binary__adds_violation(self):
        ...
        self.assertIn(
            "Line 13: Direct call to unknown binary 'ls'. "
    ...
    def test_audit__returns_expected_violations(self):
        ...
        self.assertIn(
            "Line 4: Direct call to unknown binary 'ls'",
            violations[0],
        )
        self.assertIn(
            "Line 5: Direct call to defined binary 'cat'",
            violations[1],
        )
        self.assertIn(
            "Line 7: Direct call to unknown binary 'grep'",
            violations[2],
        )
