...
class TestValidationRules(unittest.TestCase):
    ...
    def _get_errors_for_file(self, filename, rules, func_index=None):
        ...
        errors = []
        functions = (
            [parsed_file.functions[func_index]]
            if func_index is not None
            else parsed_file.functions
        )
    ...
    def test_invalid_description_modifier_variable_indent(self):
        ...
        self.assertEqual(len(errors), 3)
        self.assertIn(
            "should be in 2 space indented asterisk list format.", errors[0]
        )
    ...
    def test_invalid_type(self):
        ...
        self.assertIn(
            f"Missing or invalid type in @{Tags.ARG.name}.", errors[0]
        )
    ...
    def test_non_standard_exitcodes(self):
        ...
        for code in ["123", "124", "125", "126", "127"]:
            self.assertTrue(
                any(
                    f"Non-standard @{Tags.EXITCODE.name} {code}" in e
                    for e in errors
                )
            )
    ...
    def test_modifier_variable_sort_rule(self):
        ...
        self.assertEqual(len(errors), 1)
        self.assertIn(
            "Modifier variables should be sorted alphabetically.", errors[0]
        )
    ...
    def test_set_tag_sort_rule(self):
        ...
        self.assertEqual(len(errors), 1)
        self.assertIn(
            "tags should be sorted alphabetically by variable name.", errors[0]
        )
