...
It's also critically important to note that all functions exposed to the end user must be documented.  See the [documentation guide](#documenting-production-code) for details on the exact procedure.  Please note that private functions, not intended to be used directly by end users, are exempt from this requirement.
