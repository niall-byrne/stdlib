...
@parametrize_with_arg_combos() {
  ...
  @parametrize \
    ...
    "failing_validator__valid_var_name____implicit_source;_mock_validator_fails|_TEST_VAR;1;ARGUMENTS_KEYWORD_INVALID_DETAIL|_TEST_VAR;125;ARGUMENTS_KEYWORD_INVALID" \
    ...
    "failing_validator__valid_var_name____name_source____;_mock_validator_fails|_TEST_VAR|name;1;ARGUMENTS_KEYWORD_INVALID_DETAIL|_TEST_VAR;125;ARGUMENTS_KEYWORD_INVALID" \
    ...
    "failing_validator__valid_var_name____value_source___;_mock_validator_fails|_TEST_VAR|value;1;ARGUMENTS_KEYWORD_INVALID_DETAIL|_TEST_VAR;125;ARGUMENTS_KEYWORD_INVALID" \
...
test_stdlib_fn_keyword_assert_is_valid_with__no_kw_______@vary__returns_expected_status_code() {
  local args=()

  stdlib.array.make.from_string args "|" "${TEST_ARGS_DEFINITION}"

  _capture.rc stdlib.fn.keyword.assert.is_valid_with "${args[@]}"
...
# shellcheck disable=SC2153
test_stdlib_fn_keyword_assert_is_valid_with__no_kw_______@vary__default_message_prefix________logs_an_error() {
  local args=()
  local message_args=()
  ...
  stdlib.fn.keyword.assert.is_valid_with "${args[@]}" > /dev/null

  if [[ "${#message_args[@]}" -gt 0 ]]; then
    stdlib.logger.error.mock.assert_called_once_with \
      "1($(stdlib.__message.get "${message_args[@]}")) STDLIB_LOGGING_MESSAGE_PREFIX(${FUNCNAME[1]})"
  else
...
# shellcheck disable=SC2153
test_stdlib_fn_keyword_assert_is_valid_with__no_kw_______@vary__custom_message_prefix_________logs_an_error() {
  local args=()
  local message_args=()
  ...
  STDLIB_LOGGING_MESSAGE_PREFIX="custom_prefix" \
    stdlib.fn.keyword.assert.is_valid_with "${args[@]}" > /dev/null

  if [[ "${#message_args[@]}" -gt 0 ]]; then
    stdlib.logger.error.mock.assert_called_once_with \
      "1($(stdlib.__message.get "${message_args[@]}")) STDLIB_LOGGING_MESSAGE_PREFIX(custom_prefix)"
  else
    stdlib.logger.error.mock.assert_not_called
  fi
}
...
test_stdlib_fn_keyword_assert_is_valid_with__invalid_kw__@vary__returns_expected_status_code() {
  local args=()
...
# shellcheck disable=SC2153
test_stdlib_fn_keyword_assert_is_valid_with__invalid_kw__@vary__default_message_prefix________logs_an_error() {
  local args=()
  local message_args=()
...
# shellcheck disable=SC2153
test_stdlib_fn_keyword_assert_is_valid_with__invalid_kw__@vary__custom_message_prefix_________logs_an_error() {
  local args=()
  local message_args=()
