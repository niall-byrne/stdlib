...
stdlib.trap.__register_default_handlers() {
  stdlib.var.reserved.assert.__is_valid_with stdlib.array.assert.is_array STDLIB_CLEANUP_FN_TARGETS_ARRAY name || builtin return 123 # validates STDLIB_CLEANUP_FN_TARGETS_ARRAY
  stdlib.var.reserved.assert.__is_valid_with stdlib.array.assert.is_array STDLIB_HANDLER_ERR_FN_ARRAY name || builtin return 123     # validates STDLIB_HANDLER_ERR_FN_ARRAY
  stdlib.var.reserved.assert.__is_valid_with stdlib.array.assert.is_array STDLIB_HANDLER_EXIT_FN_ARRAY name || builtin return 123    # validates STDLIB_HANDLER_EXIT_FN_ARRAY
  stdlib.var.reserved.assert.__is_valid_with stdlib.string.assert.is_boolean STDLIB_TRACEBACK_DISABLE_BOOLEAN || builtin return 123  # validates STDLIB_TRACEBACK_DISABLE_BOOLEAN
