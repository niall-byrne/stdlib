...
stdlib.trap.__register_default_handlers() {
  STDLIB_VAR_VALIDATE_BY_NAME_BOOLEAN=1 \
    stdlib.var.assert.is_valid_with stdlib.array.assert.is_array STDLIB_CLEANUP_FN_TARGETS_ARRAY || builtin return 126 # validates STDLIB_CLEANUP_FN_TARGETS_ARRAY
  STDLIB_VAR_VALIDATE_BY_NAME_BOOLEAN=1 \
    stdlib.var.assert.is_valid_with stdlib.array.assert.is_array STDLIB_HANDLER_ERR_FN_ARRAY || builtin return 126 # validates STDLIB_HANDLER_ERR_FN_ARRAY
  STDLIB_VAR_VALIDATE_BY_NAME_BOOLEAN=1 \
    stdlib.var.assert.is_valid_with stdlib.array.assert.is_array STDLIB_HANDLER_EXIT_FN_ARRAY || builtin return 126 # validates STDLIB_HANDLER_EXIT_FN_ARRAY

  stdlib.var.assert.is_valid_with stdlib.string.assert.is_boolean STDLIB_TRACEBACK_DISABLE_BOOLEAN || builtin return 126 # validates STDLIB_TRACEBACK_DISABLE_BOOLEAN
