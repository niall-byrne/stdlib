...
#   * __STDLIB_TESTING_PROTECT_PREFIX string reserved: The prefix of the library to protect (default="stdlib").
# @noargs
# @exitcode 0 If the operation succeeded.
# @internal
_testing.__protect_stdlib() {
  builtin local stdlib_library_prefix="${__STDLIB_TESTING_PROTECT_PREFIX:-"stdlib"}" # defaults __STDLIB_TESTING_PROTECT_PREFIX
...
_testing.__protected() {
  STDLIB_BUILTIN_ALLOW_OVERRIDE_BOOLEAN=1 \
    "$(_testing.__protected_name "${1}")" "${@:2}" # defaults __STDLIB_TESTING_PROTECT_PREFIX
}
