...
# @exitcode 0 If the arrays match.
# @exitcode 1 If the arrays do not match.
...
# @exitcode 0 If the array length matches the expected length.
# @exitcode 1 If the array length does not match the expected length.
# @stderr The error message if the assertion fails.
assert_array_length() {
  if [[ $# -ne 2 ]]; then
