...
_testing.assert.message.get() {
  ...
  case "${key}" in
    ASSERT_ERROR_DID_NOT_FAIL)
      required_options=1
      message="$(_testing.gettext "The assertion '\${option1}' was expected to fail, but it succeeded instead.")"
      ;;
    ASSERT_ERROR_FILE_NOT_FOUND)
      required_options=1
      message="$(_testing.gettext "the file '\${option1}' does not exist")"
      ;;
    ASSERT_ERROR_OUTPUT_NON_MATCHING)
      required_options=0
      message="$(_testing.gettext "the expected output string was not generated")"
      ;;
    ASSERT_ERROR_OUTPUT_NULL)
      required_options=0
      message="$(_testing.gettext "the 'TEST_OUTPUT' value is empty, consider using '_capture.output'")"
      ;;
    ASSERT_ERROR_RC_NON_MATCHING)
      required_options=0
      message="$(_testing.gettext "the expected status code was not returned")"
      ;;
    ASSERT_ERROR_RC_NULL)
      required_options=0
      message="$(_testing.gettext "the 'TEST_RC' value is empty, consider using '_capture.rc'")"
      ;;
    ASSERT_ERROR_SNAPSHOT_NON_MATCHING)
      required_options=1
      message="$(_testing.gettext "the contents of '\${option1}' does not match the received output")"
      ;;
    ASSERT_ERROR_VALUE_NOT_NULL)
      required_options=1
      message="$(_testing.gettext "The value '\${option1}' is not null!")"
      ;;
    ASSERT_ERROR_VALUE_NULL)
      required_options=0
      message="$(_testing.gettext "The value is null!")"
      ;;
    ASSERT_ERROR_INSUFFICIENT_ARGS)
      required_options=1
      message="$(_testing.gettext "'\${option1}' was not given sufficient arguments")"
      ;;
    ASSERT_ERROR_ARRAY_LENGTH_NON_MATCHING)
      required_options=2
      message="$(_testing.gettext "expected [\${option1}] but was [\${option2}]")"
      ;;
    "")
      required_options=0
      return_status=126
