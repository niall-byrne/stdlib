...
_testing.mock.message.get() {
  ...
  case "${key}" in
    MOCK_CALL_ACTUAL_PREFIX)
      required_options=0
      message="$(_testing.gettext "Actual call")"
      ;;
    MOCK_CALL_N_NOT_AS_EXPECTED)
      required_options=2
      message="$(_testing.gettext "Mock '\${option1}' call \${option2} was not as expected!")"
      ;;
    MOCK_CALLED_N_TIMES)
      required_options=2
      message="$(_testing.gettext "Mock '\${option1}' was called \${option2} times!")"
      ;;
    MOCK_NOT_CALLED)
      required_options=1
      message="$(_testing.gettext "Mock '\${option1}' was not called!")"
      ;;
    MOCK_NOT_CALLED_ONCE_WITH)
      required_options=2
      message="$(_testing.gettext "Mock '\${option1}' was not called once with '\${option2}' !")"
      ;;
    MOCK_NOT_CALLED_WITH)
      required_options=2
      message="$(_testing.gettext "Mock '\${option1}' was not called with '\${option2}' !")"
      ;;
    "")
      required_options=0
      return_status=126
