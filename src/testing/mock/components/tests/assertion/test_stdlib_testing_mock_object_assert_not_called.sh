...
test_stdlib_testing_mock_object_assert_not_called__with_args_____generates_expected_log_messages() {
  ...
  stdlib.testing.internal.logger.error.mock.assert_calls_are \
    "1($(stdlib.__message.get ARGUMENT_REQUIREMENTS_VIOLATION 0 0)) STDLIB_LOGGING_MESSAGE_PREFIX(test_mock.mock.assert_not_called)" \
    "1($(stdlib.__message.get ARGUMENT_REQUIREMENTS_VIOLATION_DETAIL 1)) STDLIB_LOGGING_MESSAGE_PREFIX(test_mock.mock.assert_not_called)"
}
