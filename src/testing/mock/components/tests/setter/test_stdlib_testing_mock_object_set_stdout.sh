...
test_stdlib_testing_mock_object_set_stdout__@vary__returns_expected_status_code() {
  local args=()

  _mock.create test_mock
  stdlib.array.make.from_string args "|" "${TEST_ARGS_DEFINITION}"

  _capture.rc test_mock.mock.set.stdout "${args[@]}"
...
test_stdlib_testing_mock_object_set_stdout__@vary__generates_expected_log_messages() {
  local args=()
  ...
  for message_arg_definition in "${message_arg_definitions[@]}"; do
    ...
    expected_log_messages+=("1($(stdlib.__message.get "${message_args[@]}")) STDLIB_LOGGING_MESSAGE_PREFIX(test_mock.mock.set.stdout)")
  done

  test_mock.mock.set.stdout "${args[@]}"
...
test_stdlib_testing_mock_object_set_stdout__valid_args__@vary__generates_correct_output() {
  _mock.create test_mock
