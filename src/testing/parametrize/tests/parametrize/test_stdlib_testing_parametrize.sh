...
test_parametrize__1st_run_test_variants__duplicate_variant_____generates_correct_stderr() {
  ...
  assert_output "$(_testing.parametrize.__message.get PARAMETRIZE_PREFIX_TEST_NAME): '$(stdlib.string.colour "${STDLIB_TESTING_THEME_PARAMETRIZE_HIGHLIGHT}" "test_function_mock_@vary")'
$(_testing.parametrize.__message.get PARAMETRIZE_PREFIX_VARIANT_NAME): '$(stdlib.string.colour "${STDLIB_TESTING_THEME_PARAMETRIZE_HIGHLIGHT}" "test_function_mock_duplicate_scenario_1")'"
}
