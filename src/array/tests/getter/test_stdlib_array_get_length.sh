...
@parametrize_with_error_messages() {
  ...
  @parametrize \
    "${1}" \
    "TEST_ARGS;TEST_MESSAGE_ARG_DEFINITIONS" \
...
@parametrize_with_success_outputs() {
  ...
  @parametrize \
    ...
    "empty_array__;EMPTY_ARRAY;0" \
    "valid_array_1;TEST_ARRAY_1;3" \
    "valid_array_2;TEST_ARRAY_2;5"
}

test_stdlib_array_get_length__@vary__returns_expected_status_code() {
  local args=()
  stdlib.array.make.from_string args "|" "${TEST_ARGS}"

  _capture.rc stdlib.array.get.length "${args[@]}" > /dev/null
...
test_stdlib_array_get_length__@vary__logs_error() {
  local args=()
...
test_stdlib_array_get_length__@vary__returns_expected_output() {
  local args=()
  stdlib.array.make.from_string args "|" "${TEST_ARGS}"

  _capture.output stdlib.array.get.length "${args[@]}"
