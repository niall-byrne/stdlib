...
* STDLIB_VAR_VALIDATE_BY_NAME_BOOLEAN: A boolean value that controls whether the global variable's name (instead of value) is passed to the validation function (default=0).
* STDLIB_VAR_VALIDATE_DEFAULT_VAR: An optional variable name that can be used as a default source if the given global variable is empty or unset (default="").
