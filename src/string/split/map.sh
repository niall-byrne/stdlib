...
# shellcheck disable=SC2034
{
  ...
  STDLIB_LINE_BREAK_PLACEHOLDER=""
}

# @description Maps a function over each line of a string.
#   * STDLIB_LINE_BREAK_DELIMITER string keyword: The line break char sequence to use (default=$'\n').
#   * STDLIB_LINE_BREAK_PLACEHOLDER string keyword: A placeholder char used to encode multi-char sequences (default=$'\v').
...
stdlib.string.split.map.fn() {
  ...
  stdlib.fn.keyword.consume placeholder STDLIB_LINE_BREAK_PLACEHOLDER $'\v'

  STDLIB_KW_SOURCE_VAR="delimiter" \
    stdlib.fn.keyword.assert.is_valid_with stdlib.string.assert.not_empty STDLIB_LINE_BREAK_DELIMITER || builtin return 125 # validates STDLIB_LINE_BREAK_DELIMITER
  STDLIB_KW_SOURCE_VAR="placeholder" \
    stdlib.fn.keyword.assert.is_valid_with stdlib.string.assert.is_char STDLIB_LINE_BREAK_PLACEHOLDER || builtin return 125 # validates STDLIB_LINE_BREAK_PLACEHOLDER
  ...
  done < <(builtin echo -n "${input}${placeholder}") # KCOV_EXCLUDE_LINE
