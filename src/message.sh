...
# @description Retrieves a message by its key and optionally interpolates values.
...
# @arg $4 string (optional) Interpolation option 3.
...
stdlib.__message.get() {
  ...
  case "${key}" in
    ...
    STDIN_DEFAULT_PAUSE_PROMPT)
      required_options=0
      message="$(stdlib.__gettext "Press any key to continue ...") " # noqa
      ;;
    ...
    VAR_VALUE_INVALID)
      ...
      ;;
    VAR_VALUE_INVALID_STDIB_INTERNAL)
      required_options=1
      message="$(stdlib.__gettext "The variable '\${option1}' is reserved for internal usage in the BASH stdlib, and it has been assigned an invalid value!")"
      ;;
    "")
      required_options=0
      return_status=126
