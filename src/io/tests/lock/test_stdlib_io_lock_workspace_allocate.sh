...
test_stdlib_io_lock_workspace_allocate__@vary__invalid_perms______returns_status_code_125() {
...
test_stdlib_io_lock_workspace_allocate__@vary__invalid_perms______logs_error_message() {
  ...
  stdlib.io.path.query.is_folder.mock.set.rc "${TEST_IS_VALID_RC}"

  stdlib.io.lock.workspace_allocate

  stdlib.logger.error.mock.assert_calls_are \
    "1($(stdlib.__message.get IS_NOT_OCTAL_PERMISSION "aa"))" \
    "1($(stdlib.__message.get ARGUMENTS_KEYWORD_INVALID_DETAIL STDLIB_LOCK_WORKSPACE_PERMISSION_OCTAL))"
}
...
# shellcheck disable=SC2034
test_stdlib_io_lock_workspace_allocate__@vary__valid_args_________stops_keyword_propagation() {
  local STDLIB_LOCK_WORKSPACE="existing_value"
  ...
  assert_null "${STDLIB_LOCK_WORKSPACE_PERMISSION_OCTAL}"
}
...
test_stdlib_io_lock_workspace_allocate__no_allocated_workspace__valid_args_________success_creating_temp_______________perms_custom___calls_chmod_as_expected() {
  ...
  chmod.mock.assert_called_once_with "1(0770) 2(${STDLIB_LOCK_WORKSPACE})"
}
