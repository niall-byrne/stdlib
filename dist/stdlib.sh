...
declare -- STDLIB_LINE_BREAK_PLACEHOLDER=""
...
stdlib.fn.keyword.consume ()
{
    ...
    if stdlib.var.query.is_empty "${keyword_variable}" && stdlib.string.query.not_empty "${default_value}"; then
...
stdlib.fn.keyword.query.is_valid_with ()
{
    ...
    if ! stdlib.string.query.is_empty "${keyword_source_var}"; then
        stdlib.var.query.is_set "${keyword_source_var}" || builtin return 125;
...
stdlib.io.lock.acquire ()
{
    ...
    builtin local lock_permissions;
    builtin local polling_interval;
    builtin local quiet_acquisition_failure_boolean;
    builtin local time_elapsed;
    builtin local time_start;
    builtin local wait_time;
    stdlib.fn.args.require "1" "0" "$@" || builtin return "$?";
    stdlib.var.query.is_valid_name "${lock_name}" || builtin return 126;
    stdlib.fn.keyword.consume lock_permissions STDLIB_LOCK_PERMISSION_OCTAL "0700";
    stdlib.fn.keyword.consume polling_interval STDLIB_LOCK_POLLING_INTERVAL "0.1";
    stdlib.fn.keyword.consume quiet_acquisition_failure_boolean STDLIB_LOCK_QUIET_FAILURE_BOOLEAN "0";
    stdlib.fn.keyword.consume wait_time STDLIB_LOCK_WAIT_SECONDS "30";
...
{
    builtin local lock_workspace_permissions;
    builtin local successful_allocation_boolean=1;
    stdlib.fn.args.require "0" "0" "$@" || builtin return "$?";
    stdlib.fn.keyword.consume lock_workspace_permissions STDLIB_LOCK_WORKSPACE_PERMISSION_OCTAL "0700";
...
{
    builtin local return_code=0;
    stdlib.string.query.not_empty "${@}" || return_code="$?";
    case "${return_code}" in
        0)
...
';
    stdlib.fn.keyword.consume placeholder STDLIB_LINE_BREAK_PLACEHOLDER '';
    STDLIB_KW_SOURCE_VAR="delimiter" stdlib.fn.keyword.assert.is_valid_with stdlib.string.assert.not_empty STDLIB_LINE_BREAK_DELIMITER || builtin return 125;
    STDLIB_KW_SOURCE_VAR="placeholder" stdlib.fn.keyword.assert.is_valid_with stdlib.string.assert.is_char STDLIB_LINE_BREAK_PLACEHOLDER || builtin return 125;
    ...
    done < <(builtin echo -n "${input}${placeholder}");
    output="${output%?}";
...
stdlib.string.wrap ()
{
    ...
    stdlib.fn.keyword.consume forced_line_break_char STDLIB_LINE_BREAK_FORCE_CHAR "*";
    wrap_indent_length="${#wrap_indent_string}";
    STDLIB_KW_SOURCE_VAR="forced_line_break_char" stdlib.fn.keyword.assert.is_valid_with stdlib.string.assert.is_char STDLIB_LINE_BREAK_FORCE_CHAR || builtin return 125;
    wrap_limit="$(("${2}" - "${1}"))";
...
{
    stdlib.var.reserved.assert.__is_valid_with stdlib.array.assert.is_array STDLIB_CLEANUP_FN_TARGETS_ARRAY name || builtin return 123;
    stdlib.var.reserved.assert.__is_valid_with stdlib.array.assert.is_array STDLIB_HANDLER_ERR_FN_ARRAY name || builtin return 123;
    stdlib.var.reserved.assert.__is_valid_with stdlib.array.assert.is_array STDLIB_HANDLER_EXIT_FN_ARRAY name || builtin return 123;
    stdlib.var.reserved.assert.__is_valid_with stdlib.string.assert.is_boolean STDLIB_TRACEBACK_DISABLE_BOOLEAN || builtin return 123;
...
stdlib.trap.fn.cleanup_on_exit ()
{
    builtin local clean_up_path;
    [[ "${#@}" -eq 0 ]] || builtin return 127;
    stdlib.var.global.assert.is_valid_with stdlib.array.assert.is_array "STDLIB_CLEANUP_FN_TARGETS_ARRAY" name || builtin return 126;
...
stdlib.trap.handler.err.fn ()
{
    builtin local trap_handler_fn;
    [[ "${#@}" -eq 0 ]] || builtin return 127;
    stdlib.var.global.assert.is_valid_with stdlib.array.assert.is_array "STDLIB_HANDLER_ERR_FN_ARRAY" name || builtin return 126;
...
stdlib.trap.handler.exit.fn ()
{
    builtin local trap_handler_fn;
    [[ "${#@}" -eq 0 ]] || builtin return 127;
    stdlib.var.global.assert.is_valid_with stdlib.array.assert.is_array "STDLIB_HANDLER_EXIT_FN_ARRAY" name || builtin return 126;
